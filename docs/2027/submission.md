# Submission Policies and Instructions

This page gives the detailed submission policies for AsiaJCIS 2027. For scope, topics, and dates, see the [Call for Papers](cfp.md); posters are solicited separately in the [Call for Posters](cfp-posters.md).

!!! warning "Submission system not yet open"
    The submission site (EasyChair) and the official paper template will be linked here as soon as they are available.

## Formatting

- Papers must be written in **English** and submitted as a **PDF**.
- Use the standard **IEEE two-column conference format**, US Letter (8.5" × 11").
- **Regular papers:** up to **8 pages**, including references and appendices, with up to **2 additional pages** permitted at an extra charge.
- **Short papers:** shorter contributions reporting focused or preliminary results; the page limit will be stated with the template.
- **Posters:** up to **2 pages** (extended abstract) — see the [Call for Posters](cfp-posters.md).
- Submissions that ignore the formatting rules may be rejected without review.

## Anonymization (double-blind review)

AsiaJCIS 2027 uses **double-blind** review. Submitted manuscripts must be anonymized:

- Remove author names, affiliations, and acknowledgements.
- Refer to your own prior work in the third person (avoid "in our previous work [x]").
- Do not include links or data that reveal author identity.

## Originality and concurrent submission

Submissions must be **original** and must not substantially overlap with work that has been published, or that is simultaneously under review, at any other venue with proceedings. Submitting the same or a substantially similar paper to another venue during the AsiaJCIS review period is not permitted and is grounds for rejection.

## Plagiarism and research ethics

All submissions are screened for plagiarism and self-plagiarism, and must comply with the [IEEE plagiarism and author-ethics policies](https://www.ieee.org/publications/rights/plagiarism/plagiarism.html). Work involving human subjects, personal data, vulnerability disclosure, or other sensitive material is expected to follow responsible research practices; authors should describe how ethical concerns were handled.

## Conflicts of interest

Authors and reviewers are expected to declare conflicts of interest (e.g., recent co-authors, advisors/advisees, close collaborators, and same-institution colleagues) so that the Program Committee can assign reviews independently. Reviews are assigned across the four participating countries (Japan, China, Korea, Taiwan) to reinforce reviewer independence.

## Publication and proceedings

- **All accepted papers** are published in the **AsiaJCIS 2027 conference proceedings (via EasyChair)**.
- **Selected papers** are, instead, published through **IEEE Conference Publishing Services (CPS)** and submitted for possible inclusion in **IEEE Xplore**.
- AsiaJCIS 2027 is **not sponsored by the IEEE**. Content submitted to IEEE Xplore is subject to review based on IEEE scope and quality requirements, and **inclusion in IEEE Xplore is not guaranteed**.
- Each contribution appears in a **single** proceedings — no paper is published in more than one archival venue.
- All accepted submissions require an **IEEE copyright** assignment for the version submitted to IEEE, handled during the camera-ready process.

## Presentation requirement

At least one author of each accepted paper must **register** and give an **oral presentation** at the conference. Papers that are not presented ("no-shows") are removed from the proceedings and announced as no-show papers on the conference site. See [Registration](registration.md).

## Camera-ready

Instructions for preparing and submitting the final (camera-ready) version — template, copyright, and the IEEE PDF requirements — will be provided to authors of accepted papers after notification.
