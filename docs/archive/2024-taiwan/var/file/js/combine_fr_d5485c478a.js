...
$(".dropdown > a").off("keydown click focus blur");
$(".dropdown").on("mouseenter", function() {
const isDown = $(this).parents('ul').hasClass('dropmenu-down');
if (isPhone) return;
if (!isDown && ((isExpanMenu && isPhone) || !isExpanMenu)) {
setMenuState($(this), true);
if (isPhone || isExpanMenu) {
if ($(this).hasClass('open')) {
dropShowing = 1;
setTimeout(() => { dropShowing = 0; }, 300);
}
}
}
});
...
const $this = $(this);
const hasSubmenu = $this.children('.dropdown-menu').length > 0;
const isDown= hasSubmenu;
const shouldToggle = isPhone || (!isPhone && (isDown));
if (shouldToggle && hasSubmenu) {
if (isPhone) {
$this.siblings('.dropdown.open').each(function() {
setMenuState($(this), false);
});
toggleMenu($this);
if ($this.hasClass('open')) {
...
$(".dropdown > a").on("keydown", function(event) {
const $trigger = $(this);
const $parentLi = $trigger.closest('.dropdown');
if ($parentLi.find('> ul.dropdown-menu').length === 0) return;
switch (event.key) {
case 'Enter':
case ' ':
event.preventDefault();
...
event.preventDefault();
event.stopImmediatePropagation();
$parentLi.find('> ul.dropdown-menu > li > a').first().focus();
}
break;
}
});
$(".dropdown-menu a").on("click", function() {
if (isPhone && dropShowing) {
flag = 1;
return false;
}
});
...
if ($items.length > 0) {
$items.last().on('blur.menuDropAct', function() {
setTimeout(function() {
...
$(this).closest('.dropdown').children('a').first().focus();
}
break;
case 'Escape':
event.preventDefault();
...
$dropdown.children('a').first().focus();
break;
}
});
$("body").off("click.menuDropAct");
$('.list-group, .dropdown-menu').on("click", function(event) {
event.stopPropagation();
});
if ($('.form_date').length) {
...
const $toggle = $menu.children("a").first();
const hasSubmenu = $menu.children('.dropdown-menu').length > 0;
if (hasSubmenu) {
if (isOpen) {
$menu.addClass('open');
...
if ($link.hasClass('dropdown-toggle') || currentLi.hasClass('dropdown')) {
return;
}
...
var mockEvent = { item: { index: 0, count: e.item.count } };
updateNavLabels(mockEvent,p_id,p_left,p_right);
});
$carousel.on('changed.owl.carousel', function(e) {
updateNavLabels(e,p_id,p_left,p_right);
});
}
function updateNavLabels(event,p_id,p_left,p_right) {
var currentIndex = event.item.index;
var totalItems = event.item.count;
var prevIndex = (currentIndex - 1 + totalItems) % totalItems;
var nextIndex = (currentIndex + 1) % totalItems;
var prevAlt = $('.owl-item', p_id).eq(prevIndex).find('img').attr('alt');
var nextAlt = $('.owl-item', p_id).eq(nextIndex).find('img').attr('alt');
if(typeof(prevAlt)=='undefined') prevAlt = '';
if(typeof(nextAlt)=='undefined') nextAlt = '';
$('.owl-prev',p_id).attr('aria-label', p_left+' : '+prevAlt);
$('.owl-next',p_id).attr('aria-label', p_right+' : '+nextAlt);
}
function isIE() {
